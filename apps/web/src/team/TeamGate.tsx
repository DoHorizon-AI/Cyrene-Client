...
export function TeamGate({ children }: { children: ReactNode }) {
  ...
  async function refresh() {
    ...
    if (!response.ok) throw new Error("无法连接团队控制服务。");
  ...
  async function request(path: string, body: unknown) {
    ...
    const result = await response.json(); if (!response.ok) throw new Error(result.error?.message ?? "操作失败。"); return result;
  }
  ...
  if (!session || !session.authenticated) return <main style={{ maxWidth: 420, margin: "12vh auto", padding: 24 }}><h1>Cyrene Studio</h1><p>登录团队工作空间</p><form onSubmit={e => { e.preventDefault(); void perform(async () => { await request("login", { username, password }); setPassword(""); await refresh(); }); }}>
    <label>用户名<input aria-label="团队用户名" autoComplete="username" value={username} onChange={e => setUsername(e.target.value)} /></label>
    <label>密码<input aria-label="团队密码" type="password" autoComplete="current-password" value={password} onChange={e => setPassword(e.target.value)} /></label>
    <button disabled={busy || !session}>登录</button><button type="button" onClick={() => void perform(refresh)}>重新连接</button>
  ...
  return <SessionContext.Provider value={{ actorId: session.actor?.id ?? "local-user", workspaceId: session.actor?.workspaceIds[0] ?? "local", scopes: session.actor?.scopes ?? legacyScopes }}>{children}{session.mode === "team" && <div style={{ position: "fixed", right: 12, bottom: 2, zIndex: 50 }}>
    <button onClick={() => { setSettings(!settings); setApiToken(""); }}>{session.username} · 账号</button>
    {settings && <section aria-label="团队账号" style={{ position: "absolute", right: 0, bottom: 30, background: "#2b2d32", padding: 16, width: 360, border: "1px solid #555" }}>
      <p>工作空间：{session.actor?.workspaceIds.join(", ")}</p>
      <button disabled={busy} onClick={() => void perform(async () => { const issued = await request("tokens", { scopes: session.actor?.scopes.filter(s => s !== "team.admin") }); setApiToken(issued.token); })}>创建 MCP 凭据（24 小时）</button>
      {apiToken && <label>仅此处显示，请存入客户端环境变量<input readOnly type="password" value={apiToken} onFocus={e => e.target.select()} /></label>}
      {session.actor?.scopes.includes("team.admin") && <form onSubmit={e => { e.preventDefault(); const form = e.currentTarget, data = new FormData(form); void perform(async () => { await request("members", { username: data.get("username"), password: data.get("password"), workspaceIds: session.actor!.workspaceIds, roles: [data.get("role")] }); form.reset(); }); }}>
        <p>添加成员</p><input name="username" aria-label="新成员用户名" required /><input name="password" aria-label="新成员初始密码" type="password" minLength={12} required autoComplete="new-password" />
        <select name="role" aria-label="新成员权限"><option value="viewer">查看</option><option value="editor">编辑</option><option value="operator">执行控制</option><option value="admin">管理</option></select><button disabled={busy}>添加</button>
      </form>}
      <button disabled={busy} onClick={() => void perform(async () => { await request("logout", {}); setApiToken(""); setSettings(false); await refresh(); })}>退出登录</button>
