...
| `pipelines.redo` | 重做最近撤销的一批修改；新的普通写入会清空重做栈 |
...
当前自动验证：68 项单元/集成测试、18 项 Chrome 浏览器测试通过，TypeScript/Vite 构建通过。上游 LiteGraph eval 警告与浏览器包体积提示仍存在。

MCP 暂只开放上述 12 个流水线工具。服务器登记服务仍在原入口；节点 Product 设置、云实例操作与真实任务执行尚未迁入 MCP。没有安装远端 Agent、购买资源或启动训练。
