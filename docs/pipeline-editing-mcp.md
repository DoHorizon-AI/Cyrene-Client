...
- 尚未实现同一 graph 域内部的逐字段合并、CRDT、离线协同或多用户账户。冲突保留本地内容，供用户备份/比较后重新提交。
...
当前自动验证：68 项单元/集成测试、25 项 Chrome 浏览器测试通过，TypeScript/Vite 构建通过。回归覆盖设置代理编码路径绕过、延迟载入/导入、卸载后的回调，以及 MCP 同步期间的 Yield 写入保护。所有 Product 写接口测试使用模拟响应，不启动真实任务。上游 LiteGraph eval 警告与浏览器包体积提示仍存在。

MCP 暂只开放上述 11 个流水线工具。服务器登记服务仍在原入口；节点 Product 设置、云实例操作与真实任务执行尚未迁入 MCP。没有安装远端 Agent、购买资源或启动训练。
